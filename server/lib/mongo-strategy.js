/**
 * Passportstorage strategy
 * Used for authentificaiton an user via nodes.js passport module
 */
...
function MongoDBStrategy(dbUrl, apiKey, dbName, collection) {
  ...
  // We use the name field for the username
  LocalStrategy.call(this, { usernameField: 'name' }, this.verifyUser.bind(this));
...
MongoDBStrategy.prototype.findByName = function(name, done) {
  console.log(" MongoDBStrategy.prototype.findByName for name " + name);
  this.query({ q: JSON.stringify({name: name}) }, function(err, result) {
...
MongoDBStrategy.prototype.verifyUser = function(name, password, done) {
  this.findByName(name, function(err, user) {
    if (!err && user) {
